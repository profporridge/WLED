...
RotaryEncoderUIUsermod *rotaryEncoderUIUsermod;
#else
void *rotaryEncoderUIUsermod = nullptr;
#endif
...
private:
  ...
  uint8_t lastRotaryEncoderState = -1;
  ...
  void showTime()
  {
    ...
    if (lasttimestring != lineBuffer)
    {
      lasttimestring = String(lineBuffer);
      tft.drawString(lineBuffer, 0, 0, GFXFF);
    }
...
#define SMOOTH_FONT
    ...
    PinManagerPinType pins[] = {{TFT_WR, true}, {TFT_RD, false}, {TFT_CS, true}, {TFT_DC, true}, {TFT_RST, true}, {TFT_BL, true}, {TFT_D0, true}, {TFT_D1, false}, {TFT_D2, true}, {TFT_D3, true}, {TFT_D4, true}, {TFT_D5, true}, {TFT_D6, true}, { TFT_D7,
                                                                                                                                                                                                                                                     true }};
    if (!pinManager.allocateMultiplePins(pins, 14, PinOwner::UM_FourLineDisplay))
    {
      enabled = false;
      return;
    }
    ...
    if (!pinManager.allocateMultiplePins(pins, 7, PinOwner::UM_FourLineDisplay))
    {
      enabled = false;
      return;
    }
  ...
  void loop()
  {
    ...
    if (!displayTurnedOff && millis() - lastRedraw > 2 * 60 * 1000)
    {

#ifdef TTGO_TDISPLAY_S3
      ledcWrite(0, PIN_POWER_ON);
#else
      if (TFT_BL >= 0)
...
#ifdef USERMOD_ROTARY_ENCODER_UI
    if (rotaryEncoderUIUsermod != nullptr && lastRotaryEncoderState != rotaryEncoderUIUsermod->getCurrentState())
    {
      needRedraw = true;
      lastRotaryEncoderState = rotaryEncoderUIUsermod->getCurrentState();
    }
#endif
    if (!needRedraw)
    {
...
// Update last known values.
    ...
    if (knownSsid.length() > tftcharwidth)
      line = line.substring(0, tftcharwidth - 1) + '~';
    // center(line, tftcharwidth);
    // tft.print(line.c_str());
    xpos += tft.drawString(line.c_str(), xpos, ypos, 1);
    xpos += tft.drawString(" (", xpos, ypos, 1);
    int signalQuality = getSignalQuality(WiFi.RSSI());
    if (signalQuality < 10)
    {
    ...
    xpos += tft.drawString(String(signalQuality).c_str(), xpos, ypos, GFXFF);
    tft.setTextColor(TFT_GREEN, TFT_BLACK, true);
    xpos += tft.drawString("%)", xpos, ypos, GFXFF);
    ...
    else
    {
      ...
      tft.drawRightString(knownIp.toString().c_str(), 320, ypos, GFXFF);
    }
    xpos = 0;
    ypos += 24;
    // percent brightness
    ...
    char prefixChar = ' ';
    if (lastRotaryEncoderState == 0)
      prefixChar = '>';
    else
      prefixChar = ' ';
    sprintf(buff, PSTR("%cBrightness: %3d%%"), prefixChar, ((int)bri * 100) / 255);
    // buff[eos] =
    tft.drawString(buff, xpos, ypos, GFXFF);
    ...
    if (lastRotaryEncoderState == 1)
    {
      lineBuffer[tftcharwidth - 1] = '<';
    }
    extractModeName(knownMode, JSON_mode_names, lineBuffer, tftcharwidth);
    if (lastRotaryEncoderState == 4)
      prefixChar = '>';
    else
      prefixChar = ' ';
    sprintf(buff, PSTR("%cMode: %-20s"), prefixChar, lineBuffer);
    tft.drawString(buff, xpos, ypos, GFXFF);
    ...
    if (lastRotaryEncoderState == 3)
      prefixChar = '>';
    else
      prefixChar = ' ';
    sprintf(buff, PSTR("%cPalette: %-20s"), prefixChar, lineBuffer);
    tft.drawString(buff, xpos, ypos, GFXFF);
    ...
    tft.setCursor(xpos, ypos);
    tft.setTextColor(TFT_SILVER, TFT_BLACK, true);
    char secondPrefixChar = ' ';
    if (lastRotaryEncoderState == 1)
      prefixChar = '>';
    else
      prefixChar = ' ';
    if (lastRotaryEncoderState == 2)
      secondPrefixChar = '>';
    else
      secondPrefixChar = ' ';

    sprintf_P(buff, PSTR("FX  %cSpd:%3d %cInt:%3d"), prefixChar, effectSpeed, secondPrefixChar, effectIntensity);
    tft.drawString(buff, xpos, ypos, GFXFF);
    xpos = 0;
    ypos += 20;
    ...
    sprintf_P(buff, PSTR("Est. mA: %4d"), strip.currentMilliamps);
