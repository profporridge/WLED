...
public:
    I2SSource<T>(int sampleRate, int blockSize, int16_t lshift, uint32_t mask, float sampleScale = 1.0f) :
        AudioSource<T>(sampleRate, blockSize, lshift, mask, sampleScale) {
        _config = {
            .mode = i2s_mode_t(I2S_MODE_MASTER | I2S_MODE_RX),
            .sample_rate = AudioSource<T>::_sampleRate,                       // "narrowing conversion" warning can be ignored here - our _sampleRate is never bigger that INT32_MAX
    ...
    virtual void getSamples(T *buffer, uint16_t num_samples) {
        if(AudioSource<T>::_initialized) {
            ...
            AudioSource<T>::_dcOffset = 0.0f;              // Reset dc offset
            AudioSource<T>::_broken_samples_counter = 0;   // Reset ADC broken samples counter
            ...
            for (int i = 0; i < num_samples; i++) {
                ...
                if (AudioSource<T>::_mask == 0x0FFF) {  // mask = 0x0FFF means we are in I2SAdcSource
                    ...
                    if (AudioSource<T>::_broken_samples_counter >= num_samples-1) {             // kill-switch: ADC sample correction off when all samples in a batch were "broken"
                        AudioSource<T>::_myADCchannel = 0x0F;
                        if (serialTxAvaileable) Serial.println("AS: too many broken audio samples from ADC - sample correction switched off.");
                    }

                    newSamples[i] = (3 * sampleNoFilter + AudioSource<T>::_lastADCsample) / 4;  // apply low-pass filter (2-tap FIR)
                    //newSamples[i] = (sampleNoFilter + lastADCsample) / 2;      // apply stronger low-pass filter (2-tap FIR)
                    AudioSource<T>::_lastADCsample = sampleNoFilter;                            // update ADC last sample
                }
                ...
                T currSample = 0.0;
                if(AudioSource<T>::_shift > 0)
                  currSample = (T) (newSamples[i] >> AudioSource<T>::_shift);
                else {
                  if(AudioSource<T>::_shift < 0)
                    currSample = (T) (newSamples[i] << (- AudioSource<T>::_shift)); // need to "pump up" 12bit ADC to full 16bit as delivered by other digital mics
                  else
#ifdef I2S_SAMPLE_DOWNSCALE_TO_16BIT
                    currSample = (T) newSamples[i] / 65536.0f;        // _shift == 0 -> use the chance to keep lower 16bits
#else
                    currSample = (T) newSamples[i];
#endif
                }
    ...
    I2S_datatype decodeADCsample(I2S_unsigned_datatype rawData) {
        ...
        I2S_datatype lastGoodSample = AudioSource<T>::_lastADCsample / 16384 ;     // 26bit-> 12bit with correct sign handling
#else
        ...
        // fix bad samples
        if ((the_channel != AudioSource<T>::_myADCchannel) && (AudioSource<T>::_myADCchannel != 0x0F)) { // 0x0F means "don't know what my channel is" 
            finalSample = lastGoodSample;                         // replace with the last good ADC sample
            AudioSource<T>::_broken_samples_counter ++;
...
public:
    I2SSourceWithMasterClock(int sampleRate, int blockSize, int16_t lshift, uint32_t mask, float sampleScale = 1.0f) :
        I2SSource<T>(sampleRate, blockSize, lshift, mask, sampleScale) {
    };
...
public:
    ...
    ES7243(int sampleRate, int blockSize, int16_t lshift, uint32_t mask, float sampleScale = 1.0f) :
        I2SSourceWithMasterClock<T>(sampleRate, blockSize, lshift, mask, sampleScale) {
...
public:
    ...
    void initialize() {
        ...
        AudioSource<T>::_myADCchannel = channel;
        AudioSource<T>::_lastADCsample = 0;
    ...
    */
    ...
    void deinitialize() {
        ...
        esp_err_t err; if (AudioSource<T>::_initialized) {
         AudioSource<T>::_initialized = false;
#if defined(I2S_GRAB_ADC1_COMPLETELY)
...
public:
    I2SPdmSource(int sampleRate, int blockSize, int16_t lshift, uint32_t mask, float sampleScale = 1.0f) :
        ...
        I2SSource<T>::_config.use_apll = 1;
